...
**适用对象**：独立开发者、希望简化部署的用户

**部署方式**：
- 本地电脑直接运行 CLI 客户端（无需路由服务器）
- 客户端直接连接飞书长连接
- 通过飞书向外提供服务

**优势**：
- 🚀 **极简部署**：只需运行一个客户端，无需部署路由服务器
- ⚡ **低延迟**：消息直接从飞书到本地，无中间转发
- 🛠️ **零运维**：只需要管理一个进程
- 🆓 **完全免费**：无需服务器，个人电脑即可
...
- 可访问的**飞书机器人**（团队应部署一个路由服务器，或直连模式使用自己的机器人）

## 直连模式 - 飞书机器人配置

使用直连模式时，你需要创建并配置自己的飞书机器人：
...
7. 获取凭证（App ID、App Secret）并发布应用（发布到企业内部）
8. 在企业内找到机器人并开始对话

**注意**：直连模式只支持单用户使用，机器人会自动绑定第一个给它发消息的用户。
...
| `/abort` | 中止当前命令 |
| `/compact` | 压缩对话历史 |
| **自然语言消息** | 直接发送消息给 Claude Code |
| **其他 `/` 开头命令** | 透传给 Claude Code 执行 |
